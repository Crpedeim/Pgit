...
argparser = argparse.ArgumentParser(description="the git parser")
...
class GitRepository (object):
    ...
    def __init__(self , path, force = False):
        ...
        if cf and os.path.exists(cf):
            ...
            raise Exception("Configerations file missing")
        ...
        if not force:
            ...
            if vers != 0 :
                raise Exception("unsupported repository version: {vers}")
...
def object_read(repo , sha):
    """Read an object from a GIT repo and return an object"""
    ...
    with open(path,"rb") as f:
        ...
        if size != len(raw) - y - 1: # Checking if length is valid
...
## TODO understand ls tree command and checkout
